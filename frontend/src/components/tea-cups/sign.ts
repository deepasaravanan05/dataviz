...
/**
 * THE TEA CUPS' TEAM SIGN — "tea cups is for the (risk)".
 *
 ...
 *
 * IT IS A LABEL AND NOTHING ELSE. Risk is not a department in the roster, so
 * there is nobody to route and nothing in the simulation changes — the same as
 * the Park Train's DevOps board. Naming a team against a ride relabels the
 * ride's board in this park, and never the roster.
 *
 ...
 */
function solveTeaCupsSign(): TeamSign {
  ...
  for (const side of [1, -1]) {
    ...
    for (let out = OVERALL_REACH + 8; out <= OVERALL_REACH + 200; out += 2) {
      for (let along = -80; along <= 90; along += 5) {
        ...
        if (hidesAnotherRide(x, z, TEACUPS_RIDE_ID)) continue;
  ...
  if (!best) throw new Error("No ground beside the Tea Cups will take their signboard");

  return {
    rideId: TEACUPS_RIDE_ID,
    department: TEACUPS_TEAM_NAME,
    departments: [TEACUPS_TEAM_NAME],
    rideName: TEACUPS_RIDE_NAME,
...
export const TEACUPS_SIGN: TeamSign = solveTeaCupsSign();
